...
calcForm.addEventListener("submit", (event) => {
  ...
  const currentMaterials = getMaterialsForm();
  const woodPrice = currentMaterials.woodPrices[currentMaterials.woodType];
  const stepVolume = (tread + 0.06) * 0.06 * length;
  const stepStraightCost = woodPrice * stepVolume * 1.5;
  const stepWinderCost = stepStraightCost * currentMaterials.winderCoef;
  const stepsCost = straightSteps * stepStraightCost + winderSteps * stepWinderCost;

  const metalCost = (straightSteps * 0.44 + winderSteps * 0.66) * currentMaterials.rodPrice;
  const anchorCost = (straightSteps * 0.4 + winderSteps * 0.6) * currentMaterials.anchorPrice;

  const stepArea = tread * length;
  const totalArea = (straightSteps + winderSteps) * stepArea;
  const oilLiters = totalArea / 15;
  const oilCost = oilLiters * currentMaterials.oilPrice;

  const hypotenuse = Math.sqrt(tread ** 2 + rise ** 2);
  const railMeters = straightSteps * hypotenuse + balustradeLength;
  const railCost = railMeters * currentMaterials.railPrice;

  const landingCost = hasLanding
    ? landingWidth * landingLength * currentMaterials.landingPrice
    : 0;
